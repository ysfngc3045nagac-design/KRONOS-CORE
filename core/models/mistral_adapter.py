"""
core/models/mistral_adapter.py

interface.py'deki ModelAdapter sözleşmesinin Mistral API ile
dolduruluşu. Mistral da Groq gibi OpenAI ile uyumlu bir API sunar.

Ortam değişkeni gerekli: MISTRAL_API_KEY
Ücretsiz anahtar: https://console.mistral.ai/api-keys
"""
...
class MistralAdapter(ModelAdapter):
    def __init__(self, model: str = "mistral-small-latest", api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise RuntimeError(
                "MISTRAL_API_KEY bulunamadı. Render ortam değişkenlerine eklemen gerekiyor."
            )
        # call_id -> tool adı; Mistral'in tool mesajında "name" alanı da
        # bekleniyor, ama bize sadece tool_use_id (call_id) geliyor.
            ...
            for block in content:
                ...
                if btype == "text":
                    ...
                    tool_result_messages.append(
                        {
                            "role": "tool",
                            "name": self._call_id_to_name.get(call_id, "unknown_tool"),
