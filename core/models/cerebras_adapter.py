"""
core/models/cerebras_adapter.py

Cerebras'ın OpenAI uyumlu API'si için adaptör (çok hızlı çıkarım -
saniyede binlerce token). Otomatik yedekleme zincirinde kullanılıyor.

Ortam değişkeni: CEREBRAS_API_KEY
"""
...
class CerebrasAdapter(ModelAdapter):
    def __init__(self, model: str = "llama-3.3-70b", api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise RuntimeError(
                "CEREBRAS_API_KEY bulunamadı. Render ortam değişkenlerine eklemen gerekiyor."
            )
